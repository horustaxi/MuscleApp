...
class AdminUsuario extends Component {
  ...
  render() {
    ...
    const columns = [
      {
        ...
        render: (text, record) => <Link to={`/dashboard/admin-usuario/usuario/${record.id}`}>{text}</Link>,
    ...
    return (
      <LayoutContent>
        <PageHeader>Usuários &nbsp;&nbsp; {this.props.loading ? <Spin/> : ''}</PageHeader>
          {this.renderAlert()}
...
export default connect(mapStateToProps, mapDispatchToProps)(AdminUsuario);