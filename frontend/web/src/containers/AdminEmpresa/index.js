...
class AdminEmpresa extends Component {
  ...
  columns = [
    {
      ...
      render: (text, record) => <Link to={`/dashboard/admin-empresa/empresa/${record.id}`}>{text}</Link>,
    },
    {
  ...
  render() {
    return (
      <LayoutContent>
        <PageHeader>Empresas &nbsp;&nbsp; {this.props.loading ? <Spin/> : ''}</PageHeader>
          {this.renderAlert()}
          ...
          <Box>
            <Table
              ...
              dataSource={this.props.empresas}
            />
          </Box>
      </LayoutContent>
    );
...
export default connect(mapStateToProps, mapDispatchToProps)(AdminEmpresa);